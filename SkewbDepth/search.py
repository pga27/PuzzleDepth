...
def get_moves(pos):
	return [moves.L(pos), moves.Lp(pos), moves.B(pos),
			moves.R(pos), moves.Rp(pos), moves.Bp(pos),
			moves.U(pos), moves.Up(pos)]

# Solves the problem
def solve():
	solved_position = 'WWWWOOOOOGGGGRRRRBBBBBYYYYY'
	...
	print('Skewb Depth is ' + str(len(dist) - 2) + ', solved in ' + str(round(time.time() - start_time, 2)) + ' seconds with '+ str(total) + ' positions!')
